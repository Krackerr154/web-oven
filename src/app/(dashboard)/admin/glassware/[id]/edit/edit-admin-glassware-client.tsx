...
import { editAdminGlassware } from "@/app/actions/glassware";
import { Beaker, Save, Loader2, ArrowLeft } from "lucide-react";
import Link from "next/link";
import toast from "react-hot-toast";

export default function EditAdminGlasswareClient({ initialData }: { initialData: any }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await editAdminGlassware(initialData.id, { ...formData, quantity: quantityNum });
            if (res.success) {
                toast.success("Lab Glassware Updated!");
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p className="text-slate-400 mt-1 text-sm">
                        Update lab-owned inventory records. User items are managed by owners.
                    </p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="bg-slate-900/50 backdrop-blur-md rounded-2xl border border-slate-700/50 p-6 space-y-6 shadow-xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-slate-300">Catalog Number (Optional)</label>
                        <input
                            type="text"
                            value={formData.customId}
                            onChange={(e) => setFormData({ ...formData, customId: e.target.value })}
                            className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2.5 text-white focus:ring-2 focus:ring-orange-500/50 focus:border-orange-500 outline-none transition-all font-mono text-sm"
                            placeholder="e.g. CAT-10293"
                        />
                    </div>
                    ...
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-slate-300">Total System Actionable Quantity *</label>
                        <input
                            type="number"
                            required
                            min="0"
                            ...
                            className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2.5 text-white focus:ring-2 focus:ring-orange-500/50 focus:border-orange-500 outline-none transition-all"
                            placeholder="Total stock including presently borrowed items"
                        />
                        <p className="text-xs text-slate-500">Includes currently loaned items. Does NOT instantly return them.</p>
                    </div>
                ...
                <div className="space-y-2">
                    <label className="text-sm font-medium text-slate-300">Administrative Notes (Optional)</label>
                    <textarea
                        ...
                        placeholder="Internal lab notes, procurement details, etc."
                    />
                </div>
