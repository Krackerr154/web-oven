...
export function OvenStatusCard({ oven }: { oven: OvenData }) {
    ...
    return (
        ...
        >
            {/* Front Face */}
            <div className={cn("flex flex-col h-full flex-1 transition-opacity duration-300", showDetails ? "opacity-0 pointer-events-none" : "opacity-100")}>
            ...
            {isInUse && currentBooking && (
                <div className={cn("absolute inset-0 bg-slate-900/95 p-4 sm:p-5 flex flex-col transition-all duration-300 z-10", showDetails ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4 pointer-events-none")}>
