...
export function OvenStatusCard({ oven }: { oven: OvenData }) {
    ...
    return (
        <div
            ...
            className={`relative rounded-2xl border p-5 sm:p-6 hover-lift bg-slate-900/40 backdrop-blur-md overflow-hidden flex flex-col focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 ${isInUse ? "min-h-[240px]" : "h-full"} ${statusColor} ${isInUse ? "cursor-pointer" : ""}`}
        >
            {/* Front Face */}
            ...
            {isInUse && currentBooking && (
                <div className={`absolute inset-0 bg-slate-900/95 p-4 sm:p-5 flex flex-col transition-all duration-300 z-10 focus-within:opacity-100 focus-within:translate-y-0 ${showDetails ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4 pointer-events-none"}`}>
